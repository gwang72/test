    ...
    def fit(self, X, y):
        ...
        for _ in range(self.n_iter):
            ...
            for xi, target in zip(X, y):
                ...
                #print(xi)
                #print(delta_phi_y0_)
                #print(np.dot( np.array([xi]).T, delta_phi_y0_))
            ...
            if errors == 0 and first_nonerror:
                print('first nonerror training result after', _, 'steps.')
                first_nonerror = 0

###############################################
# various ways to modify the code
# left to be done for later interest
# 1) break out the for loop if self.errors_[_] == 0
# 2) only adjust w0_, shift0_, w1_, and shift1_ when abs(err) >= 0.5
# see multiperceptron_alter.py for more information
# pros and cons:
# the methods and be either faster to converge but make more mistakes
# or the other way round.
# multiperceptron_alter interpretes the method that applies both 1) and 2)
# multiperceptron_alter2 interpretes the method that only applies 1)
#
# multiperceptron_alter is the fatest but the least accurate
# multiperceptron is the slowest but the most accurate
# multiperceptron_alter2 is inbetween these two.
#
##############################################
        return self
